...
// 登陆表单验证正则
  ...
  resize_dom(){
      let that = this;
      //设置适配rem
      var change_rem = (($(window).width() < 1300) ? 1300 : $(window).width())/window.screen.width*100;
      document.getElementsByTagName("html")[0].style.fontSize=change_rem+"px";
      window.onresize = function(){
        let scrool_width = that.getScrollWidth();
          change_rem = (($(window).width() < 1300) ? 1300 : $(window).width())/window.screen.width*100;
          document.getElementsByTagName("html")[0].style.fontSize=change_rem+"px";
      }
  }
  getScrollWidth() {  
